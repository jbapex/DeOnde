...
const getMenuItems = (profile) => {
  ...
  if (prefix === '') {
    return [
      { key: 'crm', label: isAdmin ? 'Contatos' : 'CRM', path: '/crm', icon: BarChart3 },
    ];
  }

  const items = [
    { key: 'dashboard', label: 'Dashboard', path: `${prefix}/support`, icon: LayoutDashboard },
    { key: 'trafego', label: 'Cadastro Diário', path: `${prefix}/trafego`, icon: FileText },
    { key: 'campaigns-status', label: 'Status das Campanhas', path: `${prefix}/campaigns-status`, icon: ClipboardList },
    { key: 'apexia', label: 'ApexIA', path: '/apexia', icon: MessageSquare, disabled: profile?.role !== 'cliente' },
    { key: 'pgm-panel', label: 'Painel PGM', path: `${prefix}/pgm-panel`, icon: Activity },
    { key: 'crm', label: isAdmin ? 'Contatos' : 'CRM', path: isAdmin ? `${prefix}/crm/contatos` : `${prefix}/crm`, icon: BarChart3 },
  ];

  if (isAdmin) {
    items.splice(2, 0, { key: 'traffic-weekly', label: 'Tráfego Semanal', path: `${prefix}/traffic-weekly`, icon: TrendingUp });
  }
